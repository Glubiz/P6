...
class Messaging {
    constructor(debugNamespace, routing, dataEvent) {
        this.debug = (0, debug_1.default)(debugNamespace + ':messaging');
    ...
    send(target, type, data) {
        ...
        if (this.debug.enabled) {
            this.debug('Sending message', messageId, 'to', (0, ataraxia_transport_1.encodeId)(target));
        }
    ...
    handleData(peer, data) {
        if ((0, ataraxia_transport_1.sameId)(data.target, this.routing.self.id)) {
            // This is the target
            const lastPathEntry = data.path[data.path.length - 1];
            if (this.debug.enabled) {
                this.debug('Received message, sending ACK', lastPathEntry.id, 'to', (0, ataraxia_transport_1.encodeId)(peer.id));
            }
            ...
            if (!targetPeer || containsNode(data.path, targetPeer.id)) {
                 ...
                 */
                if (this.debug.enabled) {
                    this.debug('Unable to forward message, sending REJECT', lastPathEntry.id, 'to', (0, ataraxia_transport_1.encodeId)(peer.id));
                }
            ...
            if (this.debug.enabled) {
                this.debug('Forwarding message, sending ACK', lastPathEntry.id, 'via', messageId, 'to', (0, ataraxia_transport_1.encodeId)(peer.id));
            }
    ...
    handleAck(ack) {
        ...
        if (message.resolve) {
            ...
            if (!peer) {
                if (this.debug.enabled) {
                    this.debug('Unable to forward ACK', ack.id, 'to', (0, ataraxia_transport_1.encodeId)(previous.peer));
                }
                return;
            }
            if (this.debug.enabled) {
                this.debug('Forwarding ACK', ack.id, 'to', (0, ataraxia_transport_1.encodeId)(peer.id));
            }
    ...
    handleReject(reject) {
        ...
        if (message.reject) {
            ...
            if (!peer) {
                if (this.debug.enabled) {
                    this.debug('Unable to forward REJECT', reject.id, 'to', (0, ataraxia_transport_1.encodeId)(previous.peer));
                }
                return;
            }
            if (this.debug.enabled) {
                this.debug('Forwarding REJECT', reject.id, 'to', (0, ataraxia_transport_1.encodeId)(peer.id));
            }
...
function containsNode(path, id) {
    for (const p of path) {
        if ((0, ataraxia_transport_1.sameId)(p.node, id))
            return true;
    }
    return false;
}
